...
int main(){
    ...
    u64 state_2[4];
	u64 output_diff[4] = {0x0, 0x0, 0x0, 0x0};
    //#######################################################################
    //#######################################################################
    //#######################################################################
    int nrounds = 6;
	input_diff[0] = 0x0000000000000000;
	input_diff[1] = 0x0000000000000002;
	input_diff[2] = 0x0004000002000000;
	input_diff[3] = 0x0004000002000002;

	output_diff[0] = 0x0000000000000000;
	output_diff[1] = 0x2000000000000000;
	output_diff[2] = 0x0000000000000000;
	output_diff[3] = 0x0000000000200000;
    ...
    for(uint64_t experiment_number = 0; experiment_number < num_of_experiments; experiment_number++)
    {
        ...
        for (int i = 0; i < 4; i++)
		{
			if (output_diff[i] != (((state_1[i] ^ state_2[i])) & 0xffffffffffffffff))
			{
				flag = false;
				break;
			}
    ...
    printf("\nOutput mask:\n");
    print_knot256_state(output_diff);
    printf("\nNumber of experiments = %lu = 2^(%02d)\n", num_of_experiments, deg);
    printf("\nAbsolute correlation = %lu\n", counter);
    log_probability = (log((double)counter) / log(2)) - deg;
	printf("\nCorrelation = 2^(%0.2f)\n", log_probability);
	return 0;
}
