...
#*****************************************************************************
#       Copyright (C) 2022  <hsn.hadipour@gmail.com>
#
...
class SboxAnalyzer(SBox):
    ...
    def get_star_ddt(self, reverse=1):
        """
        ...
        for dx in range(2**self.input_size()):
            for dy in range(2**self.output_size()):
                if self.ddt[dx][dy] != 0:
                    star_ddt[dx][dy] = reverse ^ 1
                else:
                    star_ddt[dx][dy] = reverse
        return star_ddt
    ...
    def _star_ddt_to_boolean_function(self, reverse=1):
        ...
        self.star_ddt = self.get_star_ddt(reverse=reverse)
        boolean_func = dict()
        for dx in range(2**self.input_size()):
    ...
    def get_star_lat(self, reverse=1):
        """
        ...
        for lx in range(2**self.input_size()):
            for ly in range(2**self.output_size()):
                if self.lat[lx][ly] != 0:
                    star_lat[lx][ly] = reverse ^ 1
                else:
                    star_lat[lx][ly] = reverse
        return star_lat
    ...
    def _star_lat_to_boolean_function(self, reverse=1):
        ...
        self.star_lat = self.get_star_lat(reverse=reverse)
        boolean_func = dict()
        for lx in range(2**self.input_size()):
    ...
    def generate_cp_constraints(self, propagation_dictionary):
        ...
        m = len(inputs[0])
        n = len(outputs[0])
        x = [f"x{m - i - 1}" for i in range(m)]
        y = [f"y{n - i - 1}" for i in range(n)]
        last_condition = " /\\ ".join([f"y{n - i - 1} = -1" for i in range(n)])
        constraints = ""
        for i, (input, output) in enumerate(propagation_dictionary.items()):
            input_str = [f"{x[i]} == {input[i]}" for i in range(m)]
            input_str = " /\\ ".join(input_str)
            output_str = [f"{y[i]} = {output[i]}" for i in range(n)]
        ...
        for diff in range(2**input_size):
            for mask in range(2**output_size):
                ...
                for data in range(2**input_size):
                    if self.inner_product(mask, self[data]) == self.inner_product(mask, self[data ^ diff]):
                        counter_0 += 1 
                    else:
                        counter_1 += 1
